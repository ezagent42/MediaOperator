...
# ============================================
# MediaOperator launcher (wraps Claude Code)
#
# Based on agent-setup's claude.sh.tpl, with two project-local changes:
#   1. agent-setup plugin bootstrap removed — plugins are declared
#      statically in .agents/claude/settings.json, no runtime install.
#   2. --permission-mode is NOT hardcoded. It inherits from the
#      `defaultMode` in settings.json (user or project scope).
#      To override for a single run, export CLAUDE_PERMISSION_MODE
#      before invoking this script, e.g.
#        CLAUDE_PERMISSION_MODE=bypassPermissions ./media-operator.sh
#
# All modes run inside tmux for:
#   - SSH disconnect resilience (session keeps running)
#   - Seamless reattach from any terminal (./media-operator.sh)
#
# Modes:
...
[ -f "$SCRIPT_DIR/media-operator.local.sh" ] && source "$SCRIPT_DIR/media-operator.local.sh"
...
#
# Permission mode: NOT hardcoded. Inherits from defaultMode in
# settings.json. Opt-in override via CLAUDE_PERMISSION_MODE env var.
# ============================================

INTERACTIVE_FLAGS=""

if [ -n "$CLAUDE_PERMISSION_MODE" ]; then
    INTERACTIVE_FLAGS="--permission-mode $CLAUDE_PERMISSION_MODE"
fi
...
#
# Override: CLAUDE_TMUX_CLASSIC=1 ./media-operator.sh  (force plain tmux)
# ============================================
...
# If no internal args, user ran ./media-operator.sh from a new tmux window — ask interactively
if [ -z "$RUN_MODE" ]; then
...
echo "💡 Tips:"
echo "   - Reattach after disconnect: ./media-operator.sh"
if [ -n "$TMUX_CC" ]; then
    ...
    echo "   - New window: Ctrl+b, c  →  ./media-operator.sh"
